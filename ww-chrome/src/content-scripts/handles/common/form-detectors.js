...
function checkAddressForm(form) {
    ...
    const fields = [{
        ...
        {
            name: 'callingCode',
            regex: /.*(calling|country)[ _-]?(code)?.*/g
        },
        {
            name: 'region',
            regex: /.*(region).*/g
        ...
        {
            name: 'username',
            regex: /.*(username|email|mail|id).*/g
        },
        {
            name: 'email',
            regex: /.*(username|email|mail|id).*/g
    ...
    for (let i = 0; i < form.elements.length; i++) {
        ...
        for (let j = 0; j < fields.length; j++) {
            ...
            if (testTag(element, field.regex)) {
                hasFields = true;
...
function testTag(input, regex) {
    ...
    else if (input.type && typeof input.type == 'string' && regex.test(input.type.toLowerCase())) return true;
